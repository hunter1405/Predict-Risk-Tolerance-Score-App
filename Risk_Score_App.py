...
if st.button("Submit this form"):
    ...
    def reccomend(prediction):
        if load_clf.predict(df) <= 18:
            ...
            st.info("For a Aggressive level, we recommend a high-risk investment channel with high volatility in the short, medium and long term. However, the return can be significantly higher than the inflation rate. The right investment channel for you are: stocks, commodity markets, money markets") 
    st.header('Level')
    classify(prediction)
